...
import com.jdcosmetics.stockcollect.databinding.ItemSessionExportBinding
import com.jdcosmetics.stockcollect.util.DateUtils

class SessionExportAdapter(
    private val onToggle: (Long) -> Unit
) : ListAdapter<SessionEntity, SessionExportAdapter.ViewHolder>(DiffCallback) {

    private val selections = mutableSetOf<Long>()

    inner class ViewHolder(private val binding: ItemSessionExportBinding) :
        ...
        fun bind(session: SessionEntity) {
            ...
            binding.checkbox.isChecked = selections.contains(session.idSession)

            binding.root.setOnClickListener {
                if (selections.contains(session.idSession)) selections.remove(session.idSession)
                else selections.add(session.idSession)
                binding.checkbox.isChecked = selections.contains(session.idSession)
                onToggle(session.idSession)
            }
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemSessionExportBinding.inflate(
